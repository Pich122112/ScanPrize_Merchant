...
class _SignUpPageState extends State<SignUpPage> {
  ...
  Future<void> _initSmsAutofill() async {
    try {
      ...
      _codeSubscription = SmsAutoFill().code.listen(
        (code) {
          ...
          if (extractedOtp.length == 4 && mounted) {
            ...
            print('⚠️ Could not extract valid 4-digit OTP');
          }
        },
        onError: (error) {
          if (kDebugMode) {
      ...
      if (kDebugMode) {
        print('✅ SMS Auto-fill initialized successfully');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  void _onProvinceChanged(String? provinceId) {
    setState(() {
      ...
      if (provinceId != null) {
        ...
        if (kDebugMode) {
          print("📊 Found ${_districts.length} districts");
        }
      } else {
        _districts = [];
      }
    ...
    if (isFieldsValid) {
      ...
      try {
        ...
        if (signupResult['success'] == true && signupResult['data'] != null) {
          final token = signupResult['data']['authToken'];
...
//Correct with 1847 line code changes ( OTP Auto Fill )
