...
class FirebaseService {
  ...
  static bool _isDuplicateNotification(RemoteMessage message) {
    ...
    if (_processedNotificationIds.contains(notificationId)) {
      print("🔄 [DEBUG] DUPLICATE BLOCKED - ID: $notificationId");
      return true;
    }

    // Also check timing - sometimes same notification comes twice very quickly
    final now = DateTime.now();
    if (now.difference(_lastNotificationTime).inSeconds < 2) {
      print("🔄 [DEBUG] TIMING BLOCKED - Too soon after last notification");
      return true;
    }
    _lastNotificationTime = now;

    // Clean old IDs (keep only last 10 minutes worth)
    if (_processedNotificationIds.length > 20) {
      _processedNotificationIds.clear();
    }

    _processedNotificationIds.add(notificationId);
    print("✅ [DEBUG] NEW notification registered: $notificationId");
    return false;
  }

  /// Clear processed notifications (call this on app start)
...
*/
  ...
  static Future<void> init(GlobalKey<NavigatorState> navKey) async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      print("📩 [DEBUG] RAW FCM notification received: $notificationId");
      print("📩 [DEBUG] - Title: ${message.notification?.title}");
      print("📩 [DEBUG] - Body: ${message.notification?.body}");
      print("📩 [DEBUG] - Data: ${message.data}");

      // ✅ STRICT duplicate prevention - CHECK THIS FIRST
      if (_processedNotificationIds.contains(notificationId)) {
        print("🔄 [DEBUG] SKIPPING DUPLICATE - ID: $notificationId");
        return;
      }
      _processedNotificationIds.add(notificationId);
      ...
      // ✅ Process data (balance refresh)
      await handleNotificationData(message.data);

      // ✅ CRITICAL FIX: On Android, FCM automatically shows notifications in foreground
      // We should NOT show local notifications when FCM already shows them
      if (defaultTargetPlatform == TargetPlatform.android) {
        print(
          "📱 [DEBUG] Android - FCM handles notification automatically, skipping local notification",
        );
        // DO NOT call _showLocalNotification on Android
        return;
      } else {
        // For iOS, we need to show local notification manually
        print("📱 [DEBUG] iOS - Showing local notification manually");
...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  print("📩 [DEBUG] Background notification: $notificationId");
  ...
  print(
    "📱 [DEBUG] Background: FCM handles notification automatically on both platforms",
  );
}
